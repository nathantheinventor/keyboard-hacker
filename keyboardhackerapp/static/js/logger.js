/**
 * Created by jacobbrazeal on 9/14/19.
 */
...
function KeyPressEvent(keyCode, keyboardID) {
    var self = this;
...
function KeyboardLogger(pingDelay) {
    var self = this;
    ...
    self.upload = function () {
        ...
        if (self.registry.length && self.uploadIndex < self.registry.length) {
            var data = self.registry.slice(self.uploadIndex).map(x => x.offset(self.timeOffset)).map(x => x.json());
            var url = '/upload-keys';
            $.post(url, {
    ...
    self.eventHandlers = function () {
        $(document).off('keyup.logger').on('keyup.logger', function (e) {
        	const first_letter = $(".letter:not(.done):eq(0)");
			switch (e.keyCode) {
				case 8:  // backspace
					$(".letter.done:last").removeClass("done");
					break;
				case 13: // enter
					break;
				case 32: // space
					// mark done until the next space
					while ($(".letter:not(.done):eq(0)").text().charCodeAt(0) !== 160) {
						$(".letter:not(.done):eq(0)").addClass("done");
					}
					$(".letter:not(.done):eq(0)").addClass("done");
					break;
				default:
					if (first_letter.text().charCodeAt(0) !== 160) {
						first_letter.addClass("done");
					}
			}
            self.log(e.keyCode);
            this.value = '';
        ...
        $(document).off('keydown.logger').on('keydown.logger', _ => false);
    };
...
$(document).ready(function () {
    window.logger = new KeyboardLogger(500);
});
