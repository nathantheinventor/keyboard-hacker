	...
	var keyboardMap = [
		...
		function KeyboardLogger( selector ) {
			...
			self.renderComponents = function() {
				$('.keyboard-logger__in', self.getParent()).focus();
			};

			self.eventHandlers = function() {
				$('.keyboard-logger__in', self.getParent()).off('keyup.logger').on('keyup.logger', function(e) {
					self.log(e.keyCode);
					this.value = '';
				});
			};
			...
			self.render = function() {
				...
				html += '<h1>Keyboard Logger</h1>';

				html += '<input type="text" class="keyboard-logger__in" />';

				self.getParent().html(html).addClass('keyboard-logger');
